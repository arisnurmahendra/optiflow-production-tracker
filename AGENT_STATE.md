...
- Backend: access gate, audit, auth/session, RBAC permissions, sheet bootstrap/health, spreadsheet admin toolbar with project links, production append-only submit, duplicate detection, conflict quarantine, quarantine approval mutation, daily closing, adjustment, `MASTER_RECAP`, dashboard APIs, Script Properties maintenance, native test runner, and validation.
